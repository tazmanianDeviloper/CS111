...
def rot(c, n):
    """using lecture notes a new rot() function has been constructed,
with n as its skip length"""
...
def encipher(s, n):
    """Using recursion, each char in s is sent to rot() for encription,
and returned to be stored in the stored_value."""
    if len(s)==0:
        return ''
    else:
        stored_value=encipher(s[1:],n)
        return rot(s[0],n)+stored_value
...
def score_of_x(x):
    char_value=[letter_prob(char)for char in x]
    return sum(char_value)
      
#### Put your code for the decipher function below. ####
def decipher(s):
    option=[encipher(s, n) for n in range(26)]
    option2=[[score_of_x(x),x]for x in option]
    return max(option2)[1]



##    'Eph'
    
##    ccored_char_list = [[letter_prob(char), char] for char in s]
##    #bestpair = max(scored_char_list)
##    return scored_char_list
